const projects = {
	svelte: {
	    title: 'Svelte Boilerplate',
	    startDate: '',
	    endDate: '',
	    link: 'https://github.com/Nurintaaan/svelte-boilerplate',
	    responsibilities: [
	      'Boilerplate for light to medium scale project. Start your project within a second with this boilerplate',
	    ],
	    skills: [
	      {
	        name: 'svelte',
	        color: '#ffffff',
	        background: '#ff3e00',
	      }
	    ]
	 },
	 println: {
	    title: 'Println',
	    startDate: '',
	    endDate: '',
	    link: 'https://gitlab.com/PPL2018csui/Kelas-C/PPL2018-C3',
	    responsibilities: [
	      'Println is an automatic print webplatform with cashless payment system that will help user to print their files by just uploading it to the web.',
	      'Create wireframe and mock up for the web',
	      'Create admin and user facing page for the project',
	    ],
	    skills: [
	      {
	        name: 'React',
	        color: '#61DAFB',
	        background: '#222222',
	      },
	      {
	        name: 'Redux',
	        color: '#FEFEFE',
	        background: '#764ABD',
	      },
	      {
	        name: 'Jest',
	        color: '#FFFFFF',
	        background: '#C21325',
	      },
	      {
	        name: 'Figma',
	        color: '#FFFFFF',
	        background: '#000000',
	      }
	    ]
	 },
	 sevy: {
	    title: 'Sevy',
	    startDate: '',
	    endDate: '',
	    link:'https://line.me/R/ti/p/IO_w-wrX9m',
	    responsibilities: [
	      'Sevy Bot is a first private student’s financial assistant which help you to manage about your financial problem.',
	      'Creating logo, mascot and weekly publications for Sevy and increase more than twice adders since the first MVP.',
	    ],
	    skills: [
	      {
	        name: 'Adobe Illustrator',
	        color: '#330000',
	        background: '#FF9A00',
	      }
	    ]
	 },
	 learningBuddies: {
	    title: 'Learning Buddies',
	    startDate: '',
	    endDate: '',
	    link: 'https://www.youtube.com/watch?v=wYt4k_jZD_c',
	    responsibilities: [
	      'Learning buddies is a mobile application that helps people who has the same learning interest study together anytime and anyplace. With learning buddies you can find nearby events, and even find your learning partner using its feature.',
	      'Create UI with android studio',
	      'Create design and publication for the app',
	    ],
	    skills: [
	      {
	        name: 'Android Studio',
	        color: '#ffffff',
	        background: '#ff3e00',
	      },
	      {
	        name: 'XML',
	        color: '#ffffff',
	        background: '#000000',
	      }
	    ]
	 },
	 hijup: {
	 	title: 'Hijup',
	 	startDate: 'Dec 2017',
	 	endDate: 'Jan 2018',
	 	link:'',
	 	responsibilities: [
	 		'Refactoring several hijup.com main pages to implement Material UI guidelines using React and Material.io framework',
	 		'Creating component for hijup.com main pages using React, Flow and Storybook',
	 	],
	 	skills: [
	      {
	        name: 'Flow',
	        color: '#ffffff',
	        background: '#0085CA',
	      },
	      {
	        name: 'React',
	        color: '#61DAFB',
	        background: '#222222',
	      },
	      {
	        name: 'Material.io',
	        color: '#ffffff',
	        background: '#000000',
	      },
	 	]
	 },
	 kudo: {
	 	title: 'Kudo',
	 	startDate: 'Jul 2017',
	 	endDate: 'Sep 2017',
	 	link:'',
	 	responsibilities: [
	 		"Creating microservices push notification for merchant's transaction in Kudo Seller using Socket.io and node.js for the server,tornado for client and Docker for deployment",
	 		'Creating unit test using mocha and chai'
	 	],
	 	skills: [
	 	  {
	        name: 'Python',
	        color: '#FFD03F',
	        background: '#3772A4',
	      },
	      {
	        name: 'Tornado',
	        color: '#ffffff',
	        background: '#000000',
	      },
	      {
	        name: 'Docker',
	        color: '#ffffff',
	        background: '#0091E2',
	      },
	 	]
	 },
	 tokopedia: {
	 	title: 'Tokopedia',
	 	startDate: 'Jun 2018',
	 	endDate: 'Sep 2018',
	 	link:'',
	 	responsibilities: [
	 		'Working on one of the biggest unicorn in Indonesia with team that consist of 6 people to develop internal payment system using Go, Perl, NSQ and Redis'
	 	],
	 	skills: [
		  {
	        name: 'Go',
	        color: '#ffffff',
	        background: '#74CEDD',
	      },
	      {
	        name: 'NSQ',
	        color: '#ffffff',
	        background: '#6699CC',
	      },
	      {
	        name: 'Redis',
	        color: '#ffffff',
	        background: '#E90002',
	      },
	 	]
	 },
	 bukalapak: {
	 	title: 'Bukalapak',
	 	startDate: 'Feb 2019',
	 	endDate: 'Present',
	 	link:'',
	 	responsibilities: [
	 		'Scrum Master at Bukalapak squad',
	 		'Working with payment team to develop webview using vue',
	 		'Create internal Bukalapak dashboard using Vue.js and VueX',
	 		'Create unit testing using Vue-test-utils and Jest',
	 		'Create monitoring dashboard using datadog, grafana and prometheus',
	 	],
	 	skills: [
 		  {
	        name: 'Vue',
	        color: '#ffffff',
	        background: '#41B883',
	      },
	      {
	        name: 'Vuex',
	        color: '#ffffff',
	        background: '#41B883',
	      },
	      {
	        name: 'Jest',
	        color: '#FFFFFF',
	        background: '#C21325',
	      },
	 	]
	 },

}
export default projects;